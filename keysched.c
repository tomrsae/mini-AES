...
// Implementation using precomputed SBOX is
...
// GF(2⁴) multiplication table using irreversible polynomial m(x) = x⁴ + x + 1
static const unsigned char GFMP_TABLE[16][16] = {
...
// Multiply in GF(2⁴) using irreversible polynomial m(x) = x⁴ + x + 1
unsigned char multiply(unsigned char a, unsigned char b)
{
    return GFMP_TABLE[a][b];
}
...
{
    word[0] = (INVERSE_SBOX[word[0] >> 4] << 4) | INVERSE_SBOX[word[0] & 0x0F];
    word[1] = (INVERSE_SBOX[word[1] >> 4] << 4) | INVERSE_SBOX[word[1] & 0x0F];
}
...
{
    // Shift row on a 2-column matrix is its own inverse
    shift_row(word);
}
...
{
    // Mix column is its own inverse due to its
    // constant matrix being its own inverse
    mix_column(word);
}
